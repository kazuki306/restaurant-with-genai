...
回答は以下の<format></format>に示すjson形式で出力してください。１分類ごとに改行を挟んでください。
...
5. **気になるワードを抽出**:
```
以下のテキスト<text></text>から、人名、場所、体の部位の3つの分類に対応する箇所を全て抽出して、該当する分類と合わせて出力してください。
...
回答は以下の<format></format>に示すjson形式で出力してください。
